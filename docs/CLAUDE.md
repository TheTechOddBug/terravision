...
The project constitution defining core principles and technical standards is at `docs/constitution.md`.
...
**📋 Complete the full validation checklist**: See `specs/BASELINE_VALIDATION_CHECKLIST.md` for the comprehensive validation process.
...
**CRITICAL: count.index Resolution**

When `create_multiple_resources()` creates numbered instances, it deep-copies metadata from the base resource. Terraform expressions like `zone = "${tostring(count.index + 1)}"` are copied literally to ALL numbered instances - they are NOT automatically resolved per instance.

**Handling count.index in Handlers**:
- Extract instance number from resource name: `vm~2` → instance_num = 2
- Calculate count.index: `count_index = instance_num - 1` (count.index is 0-based)
- Replace and evaluate: `"${tostring(count.index + 1)}"` with count.index=1 → `"${tostring(1 + 1)}"` → `"2"`

**Example** (from Azure VM zone handler):
```python
if "count.index" in zone and "~" in vm:
    instance_num = int(vm.split("~")[1])  # vm~2 → 2
    count_index = instance_num - 1         # 2 - 1 = 1
    zone = zone.replace("count.index", str(count_index))
    # Remove Terraform wrappers and evaluate
    zone = zone.replace("${", "").replace("}", "")
    zone = zone.replace("tostring(", "").replace(")", "")
    zone = str(eval(zone))  # "1 + 1" → "2"
```
...
Attributes marked `(known after apply)` in Terraform plans appear as boolean `True` in `original_metadata`/`meta_data`. Always guard against this when reading metadata values (e.g., `availability_zone`, `id`, `arn`, `vpc_id`). Use `value is True or str(value) == "True"` to detect computed markers.
...
See `specs/EXPECTED_JSON_MODIFICATION_POLICY.md` for complete policy.
...
The resource graph (`graphdict`) is built **entirely from `terraform plan` output**, not from .tf file parsing:

1. `tf_initplan()` runs `terraform plan` → produces `tf_resources_created`
2. `setup_tfdata()` creates graph nodes from `tf_resources_created` (plan data)
3. `tf_makegraph()` adds edges from `terraform graph` output
4. `read_tfsource()` only supplements with metadata for variable resolution — it **never creates nodes**
...
- **Single-module**: When `terragrunt.hcl` is in the source directory, TerraVision delegates to `terragrunt init/plan`, then runs `terraform show/graph` directly in `.terragrunt-cache`
...
- **Binary check**: Terragrunt binary is checked lazily when a Terragrunt source is detected, not at startup
